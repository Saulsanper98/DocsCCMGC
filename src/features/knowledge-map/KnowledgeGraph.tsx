...
export function KnowledgeGraph() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error(err);
        if (!cancelled) setError('No se pudo cargar el grafo. Revisa la conexión o los permisos.');
      } finally {
  ...
  if (error) {
    return (
      <div className="flex h-[min(72vh,560px)] min-h-[360px] items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--muted)]/30 p-6 text-center text-sm text-[var(--destructive)]">
        {error}
      </div>
    );
  }
